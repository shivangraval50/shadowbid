...
export interface IShadowBidServiceStateResult { auction_count: number; commit_count: number; commitment_correlated_count: number; settled_count: number; cancelled_count: number; }
export const shadowBidServiceState = prepared<void, IShadowBidServiceStateResult>(`SELECT COUNT(*)::int AS auction_count, COUNT(*) FILTER (WHERE phase = 'COMMIT')::int AS commit_count, COUNT(*) FILTER (WHERE phase = 'COMMITMENT_CORRELATED')::int AS commitment_correlated_count, COUNT(*) FILTER (WHERE phase = 'SETTLED')::int AS settled_count, COUNT(*) FILTER (WHERE phase = 'CANCELLED')::int AS cancelled_count FROM shadowbid_auctions`);
