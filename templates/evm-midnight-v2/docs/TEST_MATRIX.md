...
| Compact privacy boundary | `shadowbid.contract.test.ts` checks persistent commitments, no salt disclosure, lifecycle circuits, and the third slot | Passed locally (binding generated with `--skip-zk`; local proving-key subprocess is blocked) |
...
| Full cross-chain/UI | `packages/tests/run-tests.ts` includes infra, DB/API, EVM cross-chain, frontend build/render/wallet tests | Not rerun: dependency symlink lacks EffectStream packages and frozen install is EPERM; no complete authoritative ShadowBid settlement flow |
