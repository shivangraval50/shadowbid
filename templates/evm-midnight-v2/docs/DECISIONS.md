...
## 2026-08-30 — Three bugs were blocking the live three-bidder E2E; all three were in the harness/reader, not in the protocol

The E2E existed but had never completed. Fixing it took three separate corrections, all found by running it against the live stack rather than by reading it:

1. **Wrong import subpath.** `live-three-bidder.ts` used `ShadowBidContract.ShadowBid.Contract` and `ShadowBidContract.witnesses` while importing `@evm-midnight/shadowbid-midnight-contract/contract`. That subpath maps to `managed/contract/index.js`, which exports `Contract`/`ledger` *directly* — no `ShadowBid` namespace and no `witnesses`. Both only exist on the package root (`_index.ts`). Failure: `undefined is not an object (evaluating 'ShadowBidContract.ShadowBid.Contract')`.
2. **`consumed_N` was never exposed.** `MidnightAuctionLedgerState` omitted the public `consumed_0/1/2` flags, so the harness's close/open-consume assertion read `undefined` and threw — even though the on-chain lifecycle had completed correctly (confirmed by dumping the live ledger directly: `commitments_closed=true`, `consumed=[true,true,true]`). These are `export ledger` fields, so exposing them leaks nothing, and SOL_REVIEW_1's H-1 remediation explicitly wants settlement eligibility able to check "committed *and consumed*".
3. **Unconditional `evm_setNextBlockTimestamp`.** The harness always nudged the chain clock to `commitDeadline + 1` before `settle()`. But Compact proving and the three `recordCommitment` transactions each mine blocks, so by that point the chain has normally already passed `commitDeadline`; the RPC rejects any timestamp at or below the latest block's, failing the run at its final step. It now nudges only when the chain has *not* already passed the deadline, and fails loudly if the settlement window has closed instead.

None of these were defects in the Compact contract, the EVM contract, or the coordinator protocol — the protocol worked on the first run that got far enough to exercise it.

## 2026-08-30 — `register_auction` makes the Compact contract single-use, so each live E2E run needs a fresh deploy

`shadowbid.compact`'s `register_auction` asserts `initialized == false`, and there are exactly three commit slots. A deployed instance therefore supports exactly one auction, permanently. Re-running the live E2E requires deploying a fresh `contract-shadowbid` instance first. When doing so against a running dev stack, deploy *only* ShadowBid and leave the retained Counter reference contract alone, so the running stack's counter address file and UI keep working. Note also that an already-running sync process reads the Compact address once at startup, so it will keep indexing the previous instance — the E2E does not depend on that, because it reads Midnight state directly through `LiveMidnightAuctionStateReader`.

## 2026-08-30 — `/api/shadowbid/demo-status` reports only what the public projection can justify

`ShadowBidApp.tsx` called this endpoint, but it did not exist and returned 404, leaving the judge Demo tab permanently in its "unavailable" state. It now derives stage status strictly from indexed public rows. `mint`/`auction` are complete once an `AuctionCreated` fact exists (sound: `createAuction` escrows the ERC-721 via `safeTransferFrom` before emitting). `commit` counts public commitment hashes only. `close`/`prove` are reported `unavailable` until settlement rather than guessed, because the Compact `commitments_closed`/`consumed_*` flags are deliberately not in the public projection; they are marked complete only once `settle()` succeeded, which is sound because settlement requires a coordinator authorization that is signed only after `validateCoordinatorDecision` confirms a closed, consumed, domain-matching ledger. The endpoint exposes no bid opening, salt, losing amount, or losing bidder identity, because none of those exist in the projection it reads. It requires a sync restart to become live, since the API router is loaded at process start.
